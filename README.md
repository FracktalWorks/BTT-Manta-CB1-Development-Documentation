...
Ignore the topmost line where it says "# DO NOT EDIT THIS FILE"

There will be a line that starts with `setenv bootargs "root=....`. Search for `loglevel=` and set the value to 3.  
...
```
 sudo nano /boot/boot.scr
```

Follow the same process as above and edit the values similarly.
...
Enter and then reboot. The changes should reflect.
